...
// CordonNode marks a node as unschedulable to prevent new pods from being scheduled on it.
// This is useful when preparing a node for eviction to prevent race conditions with the scheduler.
func CordonNode(ctx context.Context, client clientset.Interface, node *v1.Node) error {
	...
	logger.V(2).Info("Cordoning node", "node", klog.KObj(node))

	// Create a JSON patch to set Unschedulable to true
	...
	if err != nil {
		logger.Error(err, "Failed to cordon node", "node", klog.KObj(node))
		return fmt.Errorf("failed to cordon node %s: %w", node.Name, err)
	}

	logger.V(1).Info("Successfully cordoned node", "node", klog.KObj(node))
	return nil
}
...
func UncordonNode(ctx context.Context, client clientset.Interface, node *v1.Node) error {
	...
	logger := klog.FromContext(ctx)
	logger.V(2).Info("Uncordoning node", "node", klog.KObj(node))
	...
	logger.V(1).Info("Successfully uncordoned node", "node", klog.KObj(node))
	return nil
}
